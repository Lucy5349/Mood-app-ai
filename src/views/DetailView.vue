<script setup lang="ts">
import { ref, onMounted } from 'vue'
import { useRouter, useRoute } from 'vue-router'
import { useDiaryStore } from '@/stores/diary'
import { useTodoStore } from '@/stores/todo'
import { moodConfig, type DiaryItem } from '@/types/diary'

const router = useRouter()
const route = useRoute()
const store = useDiaryStore()
const todoStore = useTodoStore()

const diary = ref<DiaryItem | null>(null)
const isEditing = ref(false)
const editedContent = ref('')
const isLoading = ref(true)
const dayTodos = ref<any[]>([])

// 获取分数颜色（情绪强度越高，颜色越深）
const getScoreColor = (score: number) => {
  if (score >= 70) return 'text-red-500'
  if (score >= 50) return 'text-orange-500'
  if (score >= 30) return 'text-amber-500'
  return 'text-emerald-500'
}

// 获取分数描述（情绪强度评分）
const getScoreDescription = (score: number) => {
  if (score >= 80) return '情绪强烈'
  if (score >= 60) return '情绪较强烈'
  if (score >= 40) return '情绪一般'
  if (score >= 20) return '情绪较平静'
  return '情绪平静'
}

// 获取情绪进度条颜色（强度越高越红）
const getProgressColor = (score: number) => {
  if (score >= 70) return 'from-red-400 to-red-500'
  if (score >= 50) return 'from-orange-400 to-orange-500'
  if (score >= 30) return 'from-amber-400 to-amber-500'
  return 'from-emerald-400 to-emerald-500'
}

// 编辑日记
const startEdit = () => {
  editedContent.value = diary.value!.content
  isEditing.value = true
}

// 保存编辑
const saveEdit = async () => {
  if (editedContent.value.trim().length < 10) {
    alert('日记内容至少需要10个字符')
    return
  }
  
  const result = await store.analyzeMood(editedContent.value)
  const diaryId = diary.value!._id || diary.value!.id!
  
  await store.updateDiary(diaryId, {
    content: editedContent.value.trim(),
    moodTag: result.mood,
    score: result.score,
    aiAnalyze: result.analyze
  })
  
  diary.value = store.getDiaryById(route.params.id as string) || null
  isEditing.value = false
}

// 删除日记
const handleDelete = async () => {
  if (confirm('确定要删除这篇日记吗？')) {
    const diaryId = diary.value!._id || diary.value!.id!
    await store.deleteDiary(diaryId)
    router.push('/history')
  }
}

// 复制解压语录
const copyRelaxText = () => {
  if (diary.value?.aiAnalyze) {
    navigator.clipboard.writeText(diary.value.aiAnalyze)
    alert('已复制到剪贴板')
  }
}

onMounted(async () => {
  // 初始化 todo store
  todoStore.init()
  
  // 如果 store 中没有数据，先加载
  if (store.diaries.length === 0) {
    await store.loadFromServer()
  }
  
  diary.value = store.getDiaryById(route.params.id as string) || null
  
  if (diary.value) {
    // 加载当天的 todo
    dayTodos.value = todoStore.getTodosByDate(diary.value.date)
  }
  
  isLoading.value = false
  
  if (!diary.value) {
    router.push('/history')
  }
})
</script>

<template>
  <div class="min-h-screen pb-20" v-if="diary">
    <!-- 顶部导航 -->
    <header class="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800">
      <div class="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
        <button @click="router.back()" class="w-10 h-10 rounded-full flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
          <span class="text-xl">←</span>
        </button>
        <h1 class="text-lg font-semibold text-gray-800 dark:text-white">{{ diary.date }}</h1>
        <button 
          @click="isEditing ? saveEdit() : startEdit()"
          class="px-4 h-10 rounded-full flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          :class="isEditing ? 'bg-pink-500 text-white hover:bg-pink-600' : 'text-pink-500'"
        >
          <span v-if="isEditing" class="text-sm font-medium">保存</span>
          <span v-else class="text-xl">✏️</span>
        </button>
      </div>
    </header>

    <!-- 页面内容 -->
    <main class="max-w-2xl mx-auto px-4 py-5">
      <!-- 情绪概览 -->
      <div class="card p-5 mb-4 animate-bounce-in">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-3">
            <span class="text-4xl">{{ moodConfig[diary.moodLabel || 'calm'].emoji }}</span>
            <div>
              <p class="text-lg font-medium text-gray-800 dark:text-white">
                {{ diary.moodTag }}
              </p>
              <p class="text-xs text-gray-500 dark:text-gray-400">
                {{ getScoreDescription(diary.score) }}
              </p>
            </div>
          </div>
          <div class="relative w-20 h-20 flex-shrink-0">
            <svg class="w-full h-full transform -rotate-90">
              <circle
                cx="40" cy="40" r="36"
                class="fill-none stroke-gray-100 dark:stroke-gray-700"
                stroke-width="6"
              />
              <circle
                cx="40" cy="40" r="36"
                class="fill-none stroke-current"
                :class="getScoreColor(diary.score)"
                stroke-width="6"
                :stroke-dasharray="`${diary.score * 2.26} 226`"
                stroke-linecap="round"
              />
            </svg>
            <div class="absolute inset-0 flex items-center justify-center">
              <span class="text-xl font-bold" :class="getScoreColor(diary.score)">
                {{ diary.score }}
              </span>
            </div>
          </div>
        </div>

        <!-- AI 分析 -->
        <div v-if="diary.aiAnalyze" class="p-3 rounded-xl bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20">
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-1 flex items-center gap-1">
            <span>💭</span>
            <span>AI 情绪解读</span>
          </p>
          <p class="text-sm text-gray-700 dark:text-gray-200 leading-relaxed">{{ diary.aiAnalyze }}</p>
        </div>
      </div>

      <!-- 日记内容 -->
      <div class="card p-5 mb-4 animate-slide-up" style="animation-delay: 80ms">
        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3 flex items-center gap-2">
          <span>📖</span>
          <span>日记内容</span>
        </h3>
        
        <textarea
          v-if="isEditing"
          v-model="editedContent"
          rows="8"
          class="input-field resize-none text-sm leading-relaxed"
          placeholder="写下今天的故事..."
        ></textarea>
        <p v-else class="text-gray-700 dark:text-gray-200 whitespace-pre-wrap leading-relaxed text-sm">
          {{ diary.content }}
        </p>
        
        <p v-if="isEditing" class="mt-2 text-right text-sm" :class="editedContent.length >= 10 ? 'text-emerald-500' : 'text-gray-400'">
          {{ editedContent.length }} / 10 字符
        </p>
      </div>

      <!-- 当日完成清单 -->
      <div v-if="dayTodos.length > 0" class="card p-5 mb-4 animate-slide-up" style="animation-delay: 120ms">
        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3 flex items-center gap-2">
          <span>✅</span>
          <span>当日完成</span>
          <span class="text-xs px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400">
            {{ dayTodos.filter(t => t.completed).length }}/{{ dayTodos.length }}
          </span>
        </h3>
        <div class="space-y-2 max-h-40 overflow-y-auto">
          <div 
            v-for="(todo, index) in dayTodos" 
            :key="todo.id"
            class="flex items-center gap-3 p-2.5 rounded-xl bg-gray-50 dark:bg-gray-700/50 stagger-item"
            :style="{ animationDelay: `${index * 50}ms` }"
          >
            <div 
              class="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
              :class="todo.completed 
                ? 'bg-emerald-500 text-white' 
                : 'bg-gray-200 dark:bg-gray-600'"
            >
              <span v-if="todo.completed" class="text-xs">✓</span>
            </div>
            <span 
              class="flex-1 text-sm truncate"
              :class="todo.completed ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-200'"
            >
              {{ todo.text }}
            </span>
          </div>
        </div>
      </div>

      <!-- 治愈语录 -->
      <div v-if="diary.aiAnalyze" class="card p-5 mb-4 animate-slide-up" style="animation-delay: 160ms">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 flex items-center gap-2">
            <span>💝</span>
            <span>治愈语录</span>
          </h3>
          <button 
            @click="copyRelaxText"
            class="text-sm text-pink-500 hover:text-pink-600 transition-colors flex items-center gap-1"
          >
            <span>📋</span>
            <span>复制</span>
          </button>
        </div>
        <p class="text-gray-700 dark:text-gray-200 italic leading-relaxed text-sm">
          {{ diary.aiAnalyze }}
        </p>
      </div>

      <!-- 操作按钮 -->
      <div class="flex gap-3 animate-slide-up" style="animation-delay: 200ms">
        <button 
          v-if="isEditing"
          @click="isEditing = false"
          class="flex-1 py-3 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
        >
          取消编辑
        </button>
        <button 
          v-if="!isEditing"
          @click="handleDelete"
          class="flex-1 py-3 text-red-500 border-2 border-red-200 dark:border-red-800 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors font-medium"
        >
          🗑️ 删除日记
        </button>
      </div>

      <!-- 元信息 -->
      <div class="mt-6 text-center text-xs text-gray-400 dark:text-gray-500 animate-fade-in" style="animation-delay: 280ms">
        <p>创建于 {{ diary.createdAt }}</p>
        <p v-if="diary.updatedAt !== diary.createdAt" class="mt-1">更新于 {{ diary.updatedAt }}</p>
      </div>
    </main>

    <!-- 底部导航 -->
    <nav class="fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-t border-gray-100 dark:border-gray-800 safe-area-pb">
      <div class="max-w-2xl mx-auto flex">
        <button 
          @click="router.push('/')"
          class="flex-1 py-3 flex flex-col items-center gap-1 text-gray-400 transition-colors"
        >
          <span class="text-xl">🏠</span>
          <span class="text-xs">首页</span>
        </button>
        <button 
          @click="router.push('/editor')"
          class="flex-1 py-3 flex flex-col items-center gap-1 text-gray-400 transition-colors"
        >
          <span class="text-xl">✏️</span>
          <span class="text-xs">写日记</span>
        </button>
        <button 
          @click="router.push('/history')"
          class="flex-1 py-3 flex flex-col items-center gap-1 text-gray-400 transition-colors"
        >
          <span class="text-xl">📖</span>
          <span class="text-xs">历史</span>
        </button>
      </div>
    </nav>
  </div>
</template>
